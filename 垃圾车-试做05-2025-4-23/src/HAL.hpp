  ...
  class WaterDirectionalDriving {
    ...
    public:
        ...
        void start(float currentYaw) {
            ...
            HAL::targetYaw = targetYaw; // 更新全局目标角
            isActive = true;
...
<body>
  <div class="container">
    ...
    <div class="data-row">
      ...
      <p>目标角: <span id="targetYaw">NULL</span></p> <!-- 新增：显示目标角 -->
      <p>地理航向角: <span id="geoHeading">NULL</span></p> <!-- 新增：显示地理航向角 -->
    </div>
  ...
  <script>
        ...
        if (this.readyState == 4) {
          ...
          if (this.status == 200) {
            try {
              ...
              document.getElementById("targetYaw").innerText = data.targetYaw.toFixed(2); // 新增：更新目标角显示
              document.getElementById("geoHeading").innerText = data.geoHeading.toFixed(2); // 新增：更新地理航向角显示
            } catch (e) {
    ...
    server.on("/stopwaterdirection", HTTP_GET, [](AsyncWebServerRequest *request) {
      ...
      server.on("/sensor", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        float geoHeading = fmod(currentYaw + 180, 360); // 计算地理航向角
        
        // 直接格式化字符串，避免String对象
        snprintf(response, sizeof(response), 
            "{\"latitude\":%.6f,\"longitude\":%.6f,\"yaw\":%.2f,\"targetYaw\":%.2f,\"geoHeading\":%.2f}",
            currentLat, currentLon, currentYaw, HAL::targetYaw, geoHeading);
